...
export default function Page(param: any) {
  ...
  const steps = [
    {
      ...
      title: t("pda.inventory.put.steps.scanLocation.title"),
      description: t("pda.inventory.put.steps.scanLocation.desc"),
    },
    {
      name: "search-item",
      title: t("pda.inventory.put.steps.scanItem.title"),
      description: t("pda.inventory.put.steps.scanItem.desc"),
    },
    {
      name: "confirm-item",
      title: t("pda.inventory.put.steps.confirmItem.title"),
      description: t("pda.inventory.put.steps.confirmItem.desc"),
    },
  ];
  ...
  const handleInventorySearch = async () => {
    ...
    const json = await fetchGet("/api/wms/pda/warehouse/inventory/put-item", {
  ...
  const handleTakeItem = async (orderId: string) => {
    // handle take item
    const json = await fetchPost("/api/wms/pda/warehouse/inventory/put-item", {
      locationCode,
      orderId,
    });
    if (json.status != 200) {
  ...
  return (
    <PdaPageLayout {...prop}>
      <PdaPageTitleLayout
        icon={<IconHttpPut size={20} stroke={1} />}
        title={t("pdaMenu.inventory_put")}
      />
