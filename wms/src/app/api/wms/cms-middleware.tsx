...
export const cmsMiddleware = async (
  ...
  try {
    ...
    if (e instanceof ApiError) {
      const langCode = headers.get("lang") || "en";
      const errorCode = e.name;
      let errorMessage = e.message;
      // if (lang("error." + errorCode, langCode) != "error." + errorCode) {
      //   errorMessage = lang("error." + errorCode, langCode);
      // }
  ...
  let token = "";
  if (apiUrl.includes("/api/wms/login") || apiUrl.includes("/api/wms/token")) {
    token = returnData.data?.token;
  }

  if (apiUrl.includes("/api/wms/logout")) {
