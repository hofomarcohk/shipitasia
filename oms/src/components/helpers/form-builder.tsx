...
export function FormBuilder<TData, TValue>({
  ...
  return (
    <div className="relative w-full">
      <div
        className={cn(
          "relative grid grid-cols-2 mt-4 gap-x-4 px-1" + "lang-" + langCode
        )}
      >
        {modal?.fields.map((field: any) => {
          ...
          const fields =
            field.fields ??
            (field.template && templateFields[field.template]) ??
            [];
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 px-1 ">
                      {fields.map((subfield: any) => {
                        ...
                        return inputComponentBuilder(
                          { ...subfield, key },
                          { is_readonly: isReadOnly, ...modal.options }
                        );
                      })}
                    </div>
                  </div>
