import { ApiError } from "@/app/api/api-error";
import { getParam } from "@/app/api/api-helper";
import { wmsMiddleware } from "@/app/api/wms/wms-middleware";
import { OUTBOUND } from "@/cst/outbound";
import { updateOutboundStatus } from "@/services/outbound-order/do_update_outbound_order_status";
import { ApiReturn } from "@/types/Api";
import { NextRequest } from "next/server";

export async function PUT(request: NextRequest) {
  const body = await getParam(request);
  return wmsMiddleware(request, body, async (): Promise<ApiReturn> => {
    if (!body.orderIds || body.orderIds.length === 0) {
      throw new ApiError("MISSING_FIELD", { field: "orderIds" });
    }

    const orderStatus = OUTBOUND.STATUS_LABEL("en").find(
      (a) => a.value === body.status
    );

    if (!orderStatus) {
      throw new ApiError("INVALID_OUTBOUND_STATUS");
    }

    await updateOutboundStatus(body.orderIds ?? [], orderStatus.value);
    return {
      status: 200,
      message: "Success",
    };
  });
}
