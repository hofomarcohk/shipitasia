...
import { disableClientAccount } from "@/services/carrier/client-carrier-accounts";
...
export async function POST(
  ...
  return cmsMiddleware(request, null, async (): Promise<ApiReturn> => {
    ...
    const account = await disableClientAccount(id, {
      client_id,
      ip_address: ipOf(request),
      user_agent: request.headers.get("user-agent") ?? undefined,
    });
    return { status: 200, message: "Disabled", data: account };
  });
}
