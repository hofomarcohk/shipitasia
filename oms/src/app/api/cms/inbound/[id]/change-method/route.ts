...
import { changeInboundShippingMethod } from "@/services/inbound/method-change";
...
export async function POST(
  ...
  return cmsMiddleware(request, body, async (): Promise<ApiReturn> => {
    ...
    const data = await changeInboundShippingMethod(id, body, {
      client_id,
      ip_address: ipOf(request),
      user_agent: request.headers.get("user-agent") ?? undefined,
    });
    return { status: 200, message: "Method changed", data };
  });
}
