...
import { acceptUnclaimed } from "@/services/unclaimed/unclaimed-service";
...
export async function POST(
  request: NextRequest,
  { params }: { params: Promise<{ unclaimedId: string }> }
) {
  const { unclaimedId } = await params;
  const body = await getParam(request);
  return cmsMiddleware(request, body, async (): Promise<ApiReturn> => {
    const client_id = clientIdFromJwt(request);
    const data = await acceptUnclaimed(unclaimedId, body, {
      client_id,
      ip_address: ipOf(request),
      user_agent: request.headers.get("user-agent") ?? undefined,
    });
    return { status: 200, message: "Accepted", data };
  });
}
