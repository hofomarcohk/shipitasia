...
import { rejectUnclaimed } from "@/services/unclaimed/unclaimed-service";
...
export async function POST(
  ...
  return cmsMiddleware(request, body, async (): Promise<ApiReturn> => {
    ...
    const data = await rejectUnclaimed(unclaimedId, body, {
      client_id,
      ip_address: ipOf(request),
      user_agent: request.headers.get("user-agent") ?? undefined,
    });
    return { status: 200, message: "Rejected", data };
  });
}
